...
export const vrolijkeOpenPlekStateSchema = z.object({
  ...
  racePositions: z.record(z.string(), z.number().int().min(-1).max(24)).default({}),
  raceTurn: z.number().int().nonnegative().default(0),
  raceLastRoll: z.number().int().min(1).max(6).nullable().default(null),
  raceLastActorId: z.string().nullable().default(null),
  raceLastHitActorId: z.string().nullable().default(null),
  raceHitCounts: z.record(z.string(), z.number().int().nonnegative()).default({}),
  raceWinnerId: z.string().nullable().default(null),
...
export const vrolijkeOpenPlekActionSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    type: z.literal("vrolijke-open-plek.race.rolled"),
    actorId: z.string(),
